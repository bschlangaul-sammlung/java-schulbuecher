...
public class GLayer implements PConstants
{
    ...
    public boolean isInside(GPoint plotPoint)
    {
        return (plotPoint.isValid())
                ? isInside(plotPoint.getX(), plotPoint.getY())
                : false;
    }
        ...
        if (isInside(xPlot, yPlot))
        {
            ...
            for (int i = 0; i < nPoints; i++)
            {
                if (inside.get(i))
                {
                    ...
                    if (distSq < PApplet
                            .max(PApplet.sq(pointSizes[i % nSizes] / 2.0f), 25))
                    {
                        if (distSq < minDistSq)
                        {
                            minDistSq = distSq;
        ...
        if (plotPoint1.isValid() && plotPoint2.isValid())
        {
            ...
            if (!dontCut)
            {
                ...
                else
                {
                    ...
                    if (nCuts == 2)
                    {
                        if ((PApplet.sq(cuts[0][0] - x1) + PApplet.sq(
                                cuts[0][1] - y1)) > (PApplet.sq(cuts[1][0] - x1)
     ...
     * @param nCuts the number of cuts
     * @param tolerance maximum distance after which the points can't be
     *     duplicates
     *
    ...
    public void drawLabel(GPoint point)
    {
        ...
        if (isValidNumber(xPlot) && isValidNumber(yPlot))
        {
            ...
            parent.rect(xLabelPos - delta, yLabelPos - fontSize - delta,
                    parent.textWidth(point.getLabel()) + 2 * delta,
                    fontSize + 2 * delta);
    ...
    public void drawPolygon(GPointsArray polygonPoints, int polygonColor)
    {
        if (polygonPoints.getNPoints() > 2)
        {
            ...
            for (int i = 0; i < nPoints; i++)
            {
                ...
                boolean cond = (label.equals("single cut")
                        && nextLabel.equals("single cut"))
                        || (label.equals("single cut")
                                && nextLabel.equals("double cut"))
                        || (label.equals("double cut")
                                && nextLabel.equals("single cut"));
     ...
     *
     * @param newInside a boolean array with the information whether a point is
     *     inside or not
     */
     ...
     *
     * @param histType the new histogram type. It can be GPlot.HORIZONTAL or
     *     GPlot.VERTICAL
     */
