...
package org.bschlangaul.schulbuecher.isb.k_means.schwer.grafica.src.grafica;
...
public class GPlot implements PConstants
{
     ...
     */
    public GPlot(PApplet parent, float xPos, float yPos, float plotWidth,
            float plotHeight)
    {
        this.parent = parent;
        ...
        dim = new float[] { outerDim[0] - mar[1] - mar[3],
                outerDim[1] - mar[0] - mar[2] };
        xLim = new float[] { 0, 1 };
        yLim = new float[] { 0, 1 };
        fixedXLim = false;
        fixedYLim = false;
        ...
        mainLayer = new GLayer(this.parent, MAINLAYERID, dim, xLim, yLim, xLog,
                yLog);
    ...
    public void addLayer(String id, GPointsArray points)
    {
        ...
        if (!sameId)
        {
            GLayer newLayer = new GLayer(parent, id, dim, xLim, yLim, xLog,
                    yLog);
    ...
    public void removeLayer(String id)
    {
        ...
        {
            PApplet.println(
                    "Couldn't find a layer in the plot with id = " + id);
        }
    }
    ...
    public float[] getScreenPosAtValue(float xValue, float yValue)
    {
        ...
        float yScreen = mainLayer.valueToYPlot(yValue)
                + (pos[1] + mar[2] + dim[1]);
    ...
    public GPoint getPointAt(float xScreen, float yScreen, String layerId)
    {
        ...
        else
        {
            for (int i = 0; i < layerList.size(); i++)
            {
                ...
                {
                    float[] plotPos = getPlotPosAt(xScreen, yScreen);
                    p = layerList.get(i).getPointAtPlotPos(plotPos[0],
                            plotPos[1]);
                    break;
                }
            }
    ...
    {
        float[] plotPos = getPlotPosAt(xScreen, yScreen);
        int pointIndex = mainLayer.getPointIndexAtPlotPos(plotPos[0],
                plotPos[1]);
    ...
    {
        float[] plotPos = getPlotPosAt(xScreen, yScreen);
        int pointIndex = getLayer(layerId).getPointIndexAtPlotPos(plotPos[0],
                plotPos[1]);
    ...
    protected float[] calculatePlotXLim()
    {
        ...
        if (includeAllLayersInLim)
        {
            for (int i = 0; i < layerList.size(); i++)
            {
                float[] newLim = calculatePointsXLim(
                        layerList.get(i).getPointsRef());
        ...
        if (lim != null)
        {
            ...
            if (xLog)
            {
                ...
                {
                    delta = PApplet.exp(
                            expandLimFactor * PApplet.log(lim[1] / lim[0]));
                }
    ...
    protected float[] calculatePlotYLim()
    {
        ...
        if (includeAllLayersInLim)
        {
            for (int i = 0; i < layerList.size(); i++)
            {
                float[] newLim = calculatePointsYLim(
                        layerList.get(i).getPointsRef());
    ...
    public float[] calculatePointsXLim(GPointsArray points)
    {
        ...
        for (int i = 0; i < points.getNPoints(); i++)
        {
            if (points.isValid(i))
            {
                ...
                if (fixedYLim)
                {
                    isInside = ((yLim[1] >= yLim[0]) && (y >= yLim[0])
                            && (y <= yLim[1]))
                            || ((yLim[1] < yLim[0]) && (y <= yLim[0])
                                    && (y >= yLim[1]));
                }
    ...
    public float[] calculatePointsYLim(GPointsArray points)
    {
        ...
        for (int i = 0; i < points.getNPoints(); i++)
        {
            if (points.isValid(i))
            {
                ...
                if (fixedXLim)
                {
                    isInside = ((xLim[1] >= xLim[0]) && (x >= xLim[0])
                            && (x <= xLim[1]))
                            || ((xLim[1] < xLim[0]) && (x <= xLim[0])
                                    && (x >= xLim[1]));
                }
    ...
    public void moveHorizontalAxesLim(float delta)
    {
        ...
        if (xLog)
        {
            float deltaLim = PApplet
                    .exp(PApplet.log(xLim[1] / xLim[0]) * delta / dim[0]);
            xLim[0] *= deltaLim;
            xLim[1] *= deltaLim;
        }
        else
    ...
    public void moveVerticalAxesLim(float delta)
    {
        ...
        {
            float deltaLim = PApplet
                    .exp(PApplet.log(yLim[1] / yLim[0]) * delta / dim[1]);
            yLim[0] *= deltaLim;
            yLim[1] *= deltaLim;
        }
        else
    ...
    public void centerAndZoom(float factor, float xValue, float yValue)
    {
        ...
        {
            float deltaLim = PApplet
                    .exp(PApplet.log(xLim[1] / xLim[0]) / (2 * factor));
        ...
        {
            float deltaLim = PApplet
                    .exp(PApplet.log(yLim[1] / yLim[0]) / (2 * factor));
    ...
    public void zoom(float factor, float xScreen, float yScreen)
    {
        ...
        if (xLog)
        {
            float deltaLim = PApplet
                    .exp(PApplet.log(xLim[1] / xLim[0]) / (2 * factor));
            float offset = PApplet.exp((PApplet.log(xLim[1] / xLim[0]) / factor)
                    * (0.5f - plotPos[0] / dim[0]));
            xLim = new float[] { value[0] * offset / deltaLim,
                    value[0] * offset * deltaLim };
        }
        else
        {
            float deltaLim = (xLim[1] - xLim[0]) / (2 * factor);
        ...
        if (yLog)
        {
            float deltaLim = PApplet
                    .exp(PApplet.log(yLim[1] / yLim[0]) / (2 * factor));
            float offset = PApplet.exp((PApplet.log(yLim[1] / yLim[0]) / factor)
                    * (0.5f + plotPos[1] / dim[1]));
            yLim = new float[] { value[1] * offset / deltaLim,
                    value[1] * offset * deltaLim };
        }
        else
        {
            float deltaLim = (yLim[1] - yLim[0]) / (2 * factor);
    ...
    protected void shiftPlotPos(float[] valuePlotPos, float[] newPlotPos)
    {
        ...
        {
            float deltaLim = PApplet
                    .exp(PApplet.log(xLim[1] / xLim[0]) * deltaXPlot / dim[0]);
        ...
        {
            float deltaLim = PApplet
                    .exp(-PApplet.log(yLim[1] / yLim[0]) * deltaYPlot / dim[1]);
     ...
     *
     * @param histType the type of histogram to use. It can be GPlot.VERTICAL or
     *     GPlot.HORIZONTAL
     */
     ...
     */
    public void drawLine(GPoint point1, GPoint point2, int lineColor,
            float lineWidth)
    {
     ...
     * @param lineWidth line width
     */
    public void drawLine(float slope, float yCut, int lineColor,
            float lineWidth)
    {
     ...
     *
     * @param contourType the type of contours to use. It can be GPlot.VERTICAL
     *     or GPlot.HORIZONTAL
     ...
     *
     * @param gridType the type of grid to use. It could be GPlot.HORIZONTAL,
    ...
    public void drawGridLines(int gridType)
    {
        ...
        if (gridType == BOTH || gridType == VERTICAL)
        {
            ...
            for (int i = 0; i < xPlotTicks.size(); i++)
            {
                ...
                {
                    parent.line(xPlotTicks.get(i), 0, xPlotTicks.get(i),
                            -dim[1]);
                }
            }
        }
        ...
        if (gridType == BOTH || gridType == HORIZONTAL)
        {
            ...
            for (int i = 0; i < yPlotTicks.size(); i++)
            {
                ...
                {
                    parent.line(0, yPlotTicks.get(i), dim[0],
                            yPlotTicks.get(i));
                }
            }
        }
     ...
     * @param polygonColor the color to use to draw the polygon (contour and
     *     background)
     */
     ...
     * @param horAlign text horizontal alignment. It can be RIGHT, LEFT or
     *     CENTER
     ...
     * @param xRelativePos the plot x relative position for each layer in the
     *     plot
     * @param yRelativePos the plot y relative position for each layer in the
     *     plot
     */
    public void drawLegend(String[] text, float[] xRelativePos,
            float[] yRelativePos)
    {
        parent.pushStyle();
        ...
        {
            float[] plotPosition = new float[] { xRelativePos[i] * dim[0],
                    -yRelativePos[i] * dim[1] };
            float[] position = mainLayer.plotToValue(plotPosition[0],
                    plotPosition[1]);
            ...
            if (i == 0)
            {
                ...
                mainLayer.drawAnnotation(text[i], position[0], position[1],
                        LEFT, CENTER);
            }
            else
            {
                ...
                parent.rect(plotPosition[0] - 15, plotPosition[1], 14, 14);
                layerList.get(i - 1).drawAnnotation(text[i], position[0],
     ...
     */
    public void setMar(float bottomMargin, float leftMargin, float topMargin,
            float rightMargin)
    {
     ...
     * calculation
     *
     * @param includeAllLayers true if all layers should be considered and not
     *     only the main layer
     */
     ...
     *
     * @param histType the histogram type for the histogram in the main layer.
     *     It can be GPlot.HORIZONTAL or GPlot.VERTICAL
     */
     ...
     *
     * @param labelBgColor the label background color of the points in the main
     *     layer
     */
     ...
     *
     * @param labelSeparation the label separation of the points in the main
     *     layer
     */
     ...
     *
     * @param fontColor the color of the font that will be used in the main
     *     layer
     */
     ...
     *
     * @param fontName the name of the font that will be used in all plot
     *     elements
     * @param fontColor the color of the font that will be used in all plot
     *     elements
     * @param fontSize the size of the font that will be used in all plot
     *     elements
     */
            ...
            int fontSize)
    {
        ...
        for (int i = 0; i < layerList.size(); i++)
        {
            layerList.get(i).setAllFontProperties(fontName, fontColor,
                    fontSize);
        }
    }
     ...
     *
     * @param factor the zoom factor to increase or decrease with each mouse
     *     click
     * @param increaseButton the mouse button to increase the zoom. It could be
     *     LEFT, RIGHT or CENTER. Select CENTER to use the mouse wheel
     * @param decreaseButton the mouse button to decrease the zoom. It could be
     *     LEFT, RIGHT or CENTER. Select CENTER to use the mouse wheel
     * @param increaseKeyModifier the key modifier to use in conjunction with
     *     the increase zoom mouse button. It could be GPlot.SHIFTMOD,
     *     GPlot.CTRLMOD, GPlot.METAMOD, GPlot.ALTMOD, or GPlot.NONE if no key
     *     is needed
     * @param decreaseKeyModifier the key modifier to use in conjunction with
     *     the decrease zoom mouse button. It could be GPlot.SHIFTMOD,
     *     GPlot.CTRLMOD, GPlot.METAMOD, GPlot.ALTMOD, or GPlot.NONE if no key
     *     is needed
     */
    public void activateZooming(float factor, int increaseButton,
            int decreaseButton, int increaseKeyModifier,
            int decreaseKeyModifier)
    {
        ...
        if (increaseButton == LEFT || increaseButton == RIGHT
                || increaseButton == CENTER)
        {
            increaseZoomButton = increaseButton;
        }

        if (decreaseButton == LEFT || decreaseButton == RIGHT
                || decreaseButton == CENTER)
        {
            decreaseZoomButton = decreaseButton;
        }

        if (increaseKeyModifier == SHIFTMOD || increaseKeyModifier == CTRLMOD
                || increaseKeyModifier == METAMOD
                || increaseKeyModifier == ALTMOD || increaseKeyModifier == NONE)
        {
            increaseZoomKeyModifier = increaseKeyModifier;
        }

        if (decreaseKeyModifier == SHIFTMOD || decreaseKeyModifier == CTRLMOD
                || decreaseKeyModifier == METAMOD
     ...
     * @param keyModifier the key modifier to use in conjunction with the mouse
     *     button. It could be GPlot.SHIFTMOD, GPlot.CTRLMOD, GPlot.METAMOD,
    ...
    public void activateCentering(int button, int keyModifier)
    {
        ...
        if (keyModifier == SHIFTMOD || keyModifier == CTRLMOD
                || keyModifier == METAMOD || keyModifier == ALTMOD
                || keyModifier == NONE)
        {
    ...
    {
        if (zoomingIsActive || centeringIsActive || panningIsActive
                || labelingIsActive || resetIsActive)
        {
            int action = event.getAction();
            int button = (action == MouseEvent.WHEEL) ? CENTER
                    : event.getButton();
            int modifiers = event.getModifiers();
            float xMouse = event.getX();
            float yMouse = event.getY();
            int wheelCounter = (action == MouseEvent.WHEEL) ? event.getCount()
                    : 0;

            if (zoomingIsActive && (action == MouseEvent.CLICK
                    || action == MouseEvent.WHEEL))
            {
                if (button == increaseZoomButton
                        && (increaseZoomKeyModifier == NONE
                                || (modifiers & increaseZoomKeyModifier) != 0))
                {
                ...
                if (button == decreaseZoomButton
                        && (decreaseZoomKeyModifier == NONE
                                || (modifiers & decreaseZoomKeyModifier) != 0))
                {
                    if (isOverBox(xMouse, yMouse))
                    {
                        // Save the axes limits if it's the first mouse
            ...
            if (centeringIsActive && (action == MouseEvent.CLICK
                    || action == MouseEvent.WHEEL))
            {
                if (button == centeringButton && (centeringKeyModifier == NONE
                        || (modifiers & centeringKeyModifier) != 0))
                {
                    if (isOverBox(xMouse, yMouse))
                    {
                        // Save the axes limits if it's the first mouse
            ...
            if (panningIsActive)
            {
                if (button == panningButton && (panningKeyModifier == NONE
                        || (modifiers & panningKeyModifier) != 0))
                {
            ...
            if (labelingIsActive)
            {
                if (button == labelingButton && (labelingKeyModifier == NONE
                        || (modifiers & labelingKeyModifier) != 0))
                {
                    if ((action == MouseEvent.PRESS
                            || action == MouseEvent.DRAG)
                            && isOverBox(xMouse, yMouse))
                    {
            ...
            if (resetIsActive && (action == MouseEvent.CLICK
                    || action == MouseEvent.WHEEL))
            {
                if (button == resetButton && (resetKeyModifier == NONE
                        || (modifiers & resetKeyModifier) != 0))
                {
                    if (isOverBox(xMouse, yMouse))
                    {
