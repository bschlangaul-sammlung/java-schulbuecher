...
public class GHistogram implements PConstants
{
     ...
     * @param type the histogram type. It can be GPlot.VERTICAL or
     *     GPlot.HORIZONTAL
     * @param dim the plot box dimensions in pixels
     * @param plotPoints the points positions in the plot reference system
     */
    public GHistogram(PApplet parent, int type, float[] dim,
            GPointsArray plotPoints)
    {
        this.parent = parent;

        this.type = (type == GPlot.VERTICAL || type == GPlot.HORIZONTAL) ? type
                : GPlot.VERTICAL;
        this.dim = dim.clone();
    ...
    protected void updateArrays()
    {
        ...
        {
            leftSides.set(0,
                    (type == GPlot.VERTICAL) ? 0.2f * dim[0] : 0.2f * dim[1]);
     ...
     * Draws the histogram
     *
     * @param plotBasePoint the histogram base point in the plot reference
     *     system
     */
    public void draw(GPoint plotBasePoint)
    {
        if (visible)
        {
            ...
            {
                baseline = (type == GPlot.VERTICAL) ? plotBasePoint.getY()
                        : plotBasePoint.getX();
            }
            ...
            for (int i = 0; i < plotPoints.getNPoints(); i++)
            {
                if (plotPoints.isValid(i))
                {
                    ...
                    if (Math.abs(x2 - x1) > 2 * lw
                            && Math.abs(y2 - y1) > 2 * lw)
                    {
                        parent.rect(x1, y1, x2, y2);
                    }
                    else if ((type == GPlot.VERTICAL && x2 != x1
                            && !(y1 == y2 && (y1 == 0 || y1 == -dim[1])))
                            || (type == GPlot.HORIZONTAL && y2 != y1
                                    && !(x1 == x2
                                            && (x1 == 0 || x1 == dim[0]))))
                    {
                        parent.rect(x1, y1, x2, y2);
    ...
    protected void drawHistLabels()
    {
        ...
        if (type == GPlot.VERTICAL)
        {
            if (rotateLabels)
            {
                ...
                for (int i = 0; i < plotPoints.getNPoints(); i++)
                {
                    if (plotPoints.isValid(i) && plotPoints.getX(i) >= 0
            ...
            else
            {
                ...
                for (int i = 0; i < plotPoints.getNPoints(); i++)
                {
                    if (plotPoints.isValid(i) && plotPoints.getX(i) >= 0
                            && plotPoints.getX(i) <= dim[0])
                    {
                        parent.text(plotPoints.getLabel(i), plotPoints.getX(i),
                                labelsOffset);
                    }
                }
            }
        ...
        else
        {
            if (rotateLabels)
            {
                ...
                for (int i = 0; i < plotPoints.getNPoints(); i++)
                {
                    if (plotPoints.isValid(i) && -plotPoints.getY(i) >= 0
            ...
            else
            {
                ...
                for (int i = 0; i < plotPoints.getNPoints(); i++)
                {
                    if (plotPoints.isValid(i) && -plotPoints.getY(i) >= 0
                            && -plotPoints.getY(i) <= dim[1])
                    {
                        parent.text(plotPoints.getLabel(i), -labelsOffset,
                                plotPoints.getY(i));
                    }
                }
            }
    ...
    {
        if (newType != type
                && (newType == GPlot.VERTICAL || newType == GPlot.HORIZONTAL))
        {
            type = newType;
            updateArrays();
        }
     ...
     *
     * @param newPlotPoints the new points positions in the plot reference
     *     system
     */
