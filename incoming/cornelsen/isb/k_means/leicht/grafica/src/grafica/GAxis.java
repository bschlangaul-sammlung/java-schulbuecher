...
public class GAxis implements PConstants
{
     ...
     */
    public GAxis(PApplet parent, int type, float[] dim, float[] lim,
            boolean log)
    {
        this.parent = parent;

        this.type = (type == X || type == Y || type == TOP || type == RIGHT)
                ? type
                : X;
        this.dim = dim.clone();
        this.lim = lim.clone();
        ...
        fontName = Arrays.asList(PFont.list()).contains("SansSerif.plain")
                ? "SansSerif.plain"
                : "SansSerif";
    ...
    protected void obtainLogarithmicTicks()
    {
        ...
        ticks.set(ticks.size() - 1,
                roundPlus(PApplet.exp(lastExp * GPlot.LOG10), -lastExp));
    }
    ...
    protected void updatePlotTicks()
    {
        ...
        if (log)
        {
            ...
            {
                plotTicks.set(i,
                        PApplet.log(ticks.get(i) / lim[0]) * scaleFactor);
            }
        }
        else
        {
    ...
    protected void updateTicksInside()
    {
        ...
        if (type == X || type == TOP)
        {
            for (int i = 0; i < n; i++)
            {
                ticksInside.set(i, (plotTicks.get(i) >= 0)
                        && (plotTicks.get(i) <= dim[0]));
            }
        }
        else
        {
        ...
        if (log)
        {
            for (int i = 0; i < n; i++)
            {
                ...
                if (tick > 0)
                {
                    ...
                    boolean isExactLogValue = Math
                            .abs(logValue - Math.round(logValue)) < 0.0001;
            ...
            else
            {
                ...
                if (!fixedTicks)
                {
                    ...
                    else if (n > 0)
                    {
                        ...
                        if (ticksSeparation > 0)
                        {
                            step = (lim[1] > lim[0]) ? ticksSeparation
                                    : -ticksSeparation;
                            sigDigits = obtainSigDigits(step);
                        ...
                        {
                            step = (n == 1) ? lim[1] - lim[0]
                                    : ticks.get(1) - ticks.get(0);
                            sigDigits = obtainSigDigits(step);
                            step = roundPlus(step, sigDigits);

                            if (step == 0 || Math.abs(step) > Math
                                    .abs(lim[1] - lim[0]))
                            {
                                sigDigits++;
                                step = (n == 1) ? lim[1] - lim[0]
                            ...
                            step = (lim[1] > lim[0]) ? Math.abs(step)
                                    : -Math.abs(step);
                        }

                        // Obtain the first tick
                        float firstTick = ticks.get(0) + step
                                * PApplet.ceil((lim[0] - ticks.get(0)) / step);
                        firstTick = roundPlus(firstTick, sigDigits);

                        if ((lim[1] - firstTick) * (lim[0] - firstTick) > 0)
                        {
                            firstTick = ticks.get(0) + step * PApplet
                                    .floor((lim[0] - ticks.get(0)) / step);
                            firstTick = roundPlus(firstTick, sigDigits);
                        }

                        // Calculate the rest of the ticks
                        n = PApplet.floor(Math.abs((lim[1] - firstTick) / step))
                                + 1;
                        adaptSize(ticks, n);
                        ticks.set(0, firstTick);
                        ...
                        for (int i = 1; i < n; i++)
                        {
                            ticks.set(i, roundPlus(ticks.get(i - 1) + step,
                                    sigDigits));
                        }
                    }
    ...
    protected void drawAsXAxis()
    {
        ...
        for (int i = 0; i < plotTicks.size(); i++)
        {
            if (ticksInside.get(i))
            {
                ...
                {
                    parent.line(plotTicks.get(i), offset, plotTicks.get(i),
        ...
        if (drawTickLabels)
        {
            if (rotateTickLabels)
            {
                ...
                for (int i = 0; i < plotTicks.size(); i++)
                {
                    ...
                    {
                        parent.pushMatrix();
                        parent.translate(plotTicks.get(i),
            ...
            else
            {
                ...
                for (int i = 0; i < plotTicks.size(); i++)
                {
                    if (ticksInside.get(i) && !tickLabels.get(i).equals(""))
                    {
                        parent.text(tickLabels.get(i), plotTicks.get(i),
                                offset + tickLabelOffset);
                    }
                }
            }
    ...
    protected void drawAsYAxis()
    {
        ...
        for (int i = 0; i < plotTicks.size(); i++)
        {
            if (ticksInside.get(i))
            {
                if (log && tickLabels.get(i).equals(""))
                {
                    parent.line(-offset, plotTicks.get(i),
                            -offset - smallTickLength, plotTicks.get(i));
                }
                else
                {
                    parent.line(-offset, plotTicks.get(i), -offset - tickLength,
                            plotTicks.get(i));
                }
            }
        }
        ...
        if (drawTickLabels)
        {
            if (rotateTickLabels)
            {
                ...
                for (int i = 0; i < plotTicks.size(); i++)
                {
                    ...
                    {
                        parent.pushMatrix();
                        parent.translate(-offset - tickLabelOffset,
                                plotTicks.get(i));
                        parent.rotate(-HALF_PI);
                        parent.text(tickLabels.get(i), 0, 0);
                        parent.popMatrix();
            ...
            else
            {
                ...
                for (int i = 0; i < plotTicks.size(); i++)
                {
                    if (ticksInside.get(i) && !tickLabels.get(i).equals(""))
                    {
                        parent.text(tickLabels.get(i),
                                -offset - tickLabelOffset, plotTicks.get(i));
                    }
                }
            }
        }
        ...
        if (drawTickLabels)
        {
            ...
            else
            {
                ...
                for (int i = 0; i < plotTicks.size(); i++)
                {
                    if (ticksInside.get(i) && !tickLabels.get(i).equals(""))
                    {
                        parent.text(tickLabels.get(i), offset + tickLabelOffset,
                                plotTicks.get(i));
                    }
                }
            }
     ...
     *
     * @param newExpTickLabels true if the ticks labels should be in exponential
     *     form
     */
