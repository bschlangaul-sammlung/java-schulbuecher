...
public class Spielfigur extends Actor
{
    ...
    // A2
    String generiereVerlorenText() // liefert den Text, der bei verlorenen Spiel angezeigt werden
                                   // soll
    {
        String text;
        if (anzahlSpruenge <= 3)
    ...
    /*
     * // Aufgabe 2 // Hier die Auskommentierung entfernen, dafür die Methode zu A2
     * auskommentioeren.
     *
     * String generiereVerlorenText() { String text; if (anzahlSpruenge <= 3) { text =
     * "Du musst noch etwas üben!"; } else { text = "Das ist doch schon ganz gut!"; } return text; }
     */

    void spielVerloren() // zeigt einen Text an, wenn das Spiel verloren wurde, und beendet das
                         // Programm
    {
    ...
    void spielGewonnen() // zeigt den Text "Gewonnen!" an und stoppt das Programm
    {
    ...
    boolean beruehrtAuftrieb() // liefert true, wenn die Spielfigur eine Auftriebszone berührt
    {
        return isTouching(Auftriebzone.class);
    }

    boolean istBlockiert() // liefert true, wenn sich die Spielfigur nicht mehr weiter nach rechts
                           // bewegen kann
    {
        return isTouching(Boden.class) && getY() > 155;
    }

    boolean beruehrtBoden() // liefert ture, wenn die Spielfigur den unteren Rand der Welt berührt
    {
        return isTouching(Boden.class) && getY() <= 155;
    }
}
