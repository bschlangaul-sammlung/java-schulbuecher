...
public class Tierwelt extends World
{
    public Tierwelt()
    {
        super(15, 4, 60); // erzeugt eine Welt mit 15x4 Zellen der Seitenlänge 60
        erstelleHintergrund(); // fügt Wasser, Gras und Steine in die Welt ein

        addObject(new Biene(), 2, 1); // erzeugt ein Objekt der Klasse Biene und fügt es an der
                                      // Stelle (2|1) ein
        addObject(new Eisbaer(), 2, 2); // erzeugt ein Objekt der Klasse Eisbaer und fügt es an der
                                        // Stelle (2|2) ein
        addObject(new Fisch(), 2, 3); // erzeugt ein Objekt der Klasse Fisch und fügt es an der
                                      // Stelle (2|3) ein
