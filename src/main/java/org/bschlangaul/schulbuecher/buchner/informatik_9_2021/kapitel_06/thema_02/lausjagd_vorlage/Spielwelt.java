package org.bschlangaul.schulbuecher.buchner.informatik_9_2021.kapitel_06.thema_02.lausjagd_vorlage;
...
public class Spielwelt extends World
{
    ...
    public Spielwelt()
    {
        super(800, 600, 1); // erzeugt eine Welt, 800x600 Zellen, Seitenlänge jeweils 1 Pixel
        addObject(new Marienkaefer(this), 300, 300); // fügt Marienkäfer der Welt hinzu
    ...
    void fuegeBlattlausEin()
    {
        int xPosition;
        int yPosition;
        xPosition = Greenfoot.getRandomNumber(600) + 100; // verhindert, dass die Laus in die linke
                                                          // Hecke gesetzt wird
        yPosition = Greenfoot.getRandomNumber(600);
        // Aufgabe 1
        // verwende die Methode berechneDistanzZurNaehestenBlattlaus(xPosition, yPosition)


        addObject(new Blattlaus(), xPosition, yPosition); // fügt eine Blattlaus an der Stelle
                                                          // (xPosition|yPosition) ein
    }
    ...
    public void act() // erhöht nach 1200 Zeitschritten die Zahl der Blattläuse wiedr auf 30
    {
        timer = timer - 1;
        if (timer <= 0)
        {
    ...
    double berechneDistanzZurNaehestenBlattlaus(int x, int y)
    {
        ...
        {
            Blattlaus aktuelleLaus = getObjects(Blattlaus.class).get(blattlausNr);
