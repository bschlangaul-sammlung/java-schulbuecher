...
class FENSTER extends Frame implements KeyListener
{
     ...
     * Um die Werte für die Ränder zu ermitteln, ist es notwendig, ein Objekt der Klasse Insets zu
     * erzeugen. <br>
     * Insets raender; <br>
     * raender = new Insets(0,0,0,0); <br>
     * Die Werte können mit Hilfe der Methode getInsets() ausgelesen werden. <br>
     * raender = getInsets(); <br>
     * Aus dem Insets-Objekt können die Werte der Ränder ausgelesen werden. Beispiel: <br>
     * raender.left; <br>
    ...
    FENSTER(int breite, int hoehe, DARSTELLUNG d)
    {
        ...
        setSize(breite + raender.left + raender.right, hoehe + raender.top + raender.bottom);
    ...
    /**
     * Ereignisroutine bei gedrückter Taste. Wird die "Pfeil-nach-links-Taste" (VK_LEFT) gedrückt,
     * so wird an die Darstellung eine Richtungsänderung gemeldet und der Wert 'l' übergeben. Wird
     * die "Pfeil-nach-rechts-Taste" (VK_RIGHT) gedrückt, so wird an die Darstellung eine
     * Richtungsänderung gemeldet und der Wert 'r' übergeben. * @return Oberer Rand des
     * Ausgabefensters.
     */
    ...
    /**
     * Nicht erforderlich in diesem Projekt
     */
