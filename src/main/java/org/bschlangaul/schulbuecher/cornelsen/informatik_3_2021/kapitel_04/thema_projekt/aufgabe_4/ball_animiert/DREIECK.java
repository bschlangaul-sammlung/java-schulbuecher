...
public class DREIECK extends Dreieck
{
    ...
    /**
     * Konstruktor der Klasse {@code DREIECK}. Erstellt ein neues
     * Standard-Dreieck.
     */
    public DREIECK()
    {
    ...
    /**
     * Konstruktor der Klasse {@code DREIECK}. Erstellt ein neues Dreieck mit
     * gegebenen Eck-Koordinaten. Die Reihenfolge der Ecken ist egal.
     *
     ...
     */
    public DREIECK(double Ax, double Ay, double Bx, double By, double Cx,
            double Cy)
    {
        ...
        this.C_x = Cx;
        this.C_y = Cy;
        this.M_x = (Math.min(Math.min(Ax, Bx), Cx)
                + Math.max(Math.max(Ax, Bx), Cx)) / 2;
        this.M_y = (Math.min(Math.min(Ay, By), Cy)
                + Math.max(Math.max(Ay, By), Cy)) / 2;
        this.sichtbar = true;
     ...
     * ist -1. Jedes Objekt wird zunaechst in Ebene 0 erzeugt.
     *
     *
     ...
     */
    public void setzeEcken(double Ax, double Ay, double Bx, double By,
            double Cx, double Cy)
    {
        ...
        ea.Vector[] points = { new ea.Vector(Ax, Ay), new ea.Vector(Bx, By),
                new ea.Vector(Cx, Cy) };
     ...
     * <b>ACHTUNG!</b><br />
     * Dies ist <i>nicht</i> der geometrische Mittelpunkt. Denkt man sich ein
     * Rechteck, das man "genau ueber das Dreieck" spannt, sodass es dieses
     * gerade von allen Seiten umschliesst, so ist der Mittelpunkt <b>dieses
     * Rechtecks</b> der, der hier neu gesetzt wird.
     *
     ...
     *
     * @param sichtbarNeu Ist dieser Wert {@code true}, ist nach dem Aufruf
     *     dieser Methode dieses Dreieck sichtbar. Ist dieser Wert
     *     {@code false}, so ist nach dem Aufruf dieser Methode dieses Dreieck
     *     unsichtbar.
     */
    ...
    /**
     * Verschiebt dieses Dreieck um eine gewisse Strecke - angegeben durch ein
     * "Delta X" und "Delta Y" (in Bildschirm-Metern).
     *
    ...
    /**
     * Diese Methode gibt die x-Koordinate des Mittelpunkts dieses Dreiecks (in
     * Bildschrim-Metern) zurueck.
     *
     * @return Die aktuelle x-Koordinate des Mittelpunktes dieses Dreiecks (in
     *     Bildschrim-Metern)
     */
    ...
    /**
     * Diese Methode gibt die y-Koordinate des Mittelpunkts dieses Dreiecks (in
     * Bildschrim-Metern) zurueck.
     *
     * @return Die aktuelle y-Koordinate des Mittelpunktes dieses Dreiecks (in
     *     Bildschrim-Metern)
     */
    ...
    /**
     * Diese Methode prueft, wie weit der Mittelpunkt dieses Dreiecks vom
     * Mittelpunkt eines anderen Grafik-Objekts in x-Richtung (in
     * Bildschrim-Metern) entfernt ist.
     *
     * @param ea Das andere Grafik-Objekt
     *
     * @return Abstand (in Bildschrim-Metern) dieses Dreiecks vom anderen
     *     Grafik-Objekt in x-Richtung (>0, wenn dieses Rechteck rechts des
     *     anderen Grafik-Objekts liegt)
     */
    ...
    /**
     * Diese Methode prueft, wie weit der Mittelpunkt dieses Dreiecks vom
     * Mittelpunkt eines anderen Grafik-Objekts in y-Richtung (in
     * Bildschrim-Metern) entfernt ist.
     *
     * @param ea Das andere Grafik-Objekt
     *
     * @return Abstand (in Bildschrim-Metern) dieses Dreiecks vom anderen
     *     Grafik-Objekt in y-Richtung (>0, wenn dieses Dreieck unterhalb des
     *     anderen Grafik-Objekts liegt)
     */
     ...
     *     positiver Drehsinn (gegen den Uhrzeigersinn) -: mathematisch
