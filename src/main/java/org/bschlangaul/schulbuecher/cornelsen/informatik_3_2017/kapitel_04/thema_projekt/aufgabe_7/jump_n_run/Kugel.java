...
public class Kugel
{
    ...
    /**
     * Sorgt dafuer, dass die Kugel neu geworfen wird, wenn sie ganz nach unten gelangt ist oder
     * still liegt.
     *
     */
    private void wiederholen()
    {
        while (true)
        {
            if (this.kugel.nenneMittelpunktY() < -12
                            || (Math.abs(this.kugel.nenneGeschwindigkeitX()) < 0.1
                                            && Math.abs(this.kugel.nenneGeschwindigkeitY()) < 0.1))
            {
                ...
                if (this.nachRechts)
                {
                    this.kugel.wirkeImpuls(SPIEL.zufallszahlVonBis(300, 500), 0);
                }
                else
                {
                    this.kugel.wirkeImpuls(-SPIEL.zufallszahlVonBis(100, 300), 0);
                }
            }
            SPIEL.warte(0.1);
        }
