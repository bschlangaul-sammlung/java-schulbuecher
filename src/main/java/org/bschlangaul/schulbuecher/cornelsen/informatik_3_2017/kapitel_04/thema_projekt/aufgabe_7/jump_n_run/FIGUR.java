...
/**
 * Klasse FIGUR repreasentiert eine Spielfigur, deren Grafik animiert ist. Die Animationen koennen
 * durch ein animiertes GIF, durch Einzelbilder oder durch Spritesheets erzeugt werden. Jede Figur
 * kann mehrere Zustaende annehmen: Je einen fuer eine Animationsfolge. So kann man z.B. eine andere
 * Animationsfolge fuer 'laufen' verwenden als fuer 'springen' ...
 *
 * @author mike_gans@yahoo.de and michael andonie
 *
 * @version 4.0 (2019-08-06)
 *
 * @changelog 4.0 Umstieg auf EA 4 Nicht vergleichbar mit alter Klasse FIGUR (kein Figuren-Editor
 *            mehr). Vereint die alten Klassen FIGUR, BILD und WECHSELBILD. Kann ein Bild, mehrere
 *            Bilder, Sprite-Sheets und animierte GIFs einlesen.
 */
public class FIGUR extends Figur
{
    ...
    /**
     * Konstruktor der Klasse FIGUR <b>fuer ein animiertes GIF</b>. Es wird ein erster Zustand
     * angelegt mit der Animation des GIF.
     *
     * @param zustandName Frei waehlbarer Name des Zustands. (Wird zum Umschalten verwendet)
     * @param gifPfad Datei-Pfad zu einem animierten GIF. (Liegt dieses direkt im Projekt-Ordner, so
     ...
     * Einfache Bilder koennen mit x=1 und y=1 geladen werden. Sprites muessen alle gleich gross
     * sein !!! Es wird ein erster Zustand angelegt mit der Animation des Spritesheets.
     *
     *
     * @param zustandName Frei waehlbarer Name des Zustands. (Wird zum Umschalten verwendet)
     * @param spritesheetPfad Datei-Pfad zu einem Spritesheet oder Bild. (Liegt dieses direkt im
     *        Projekt-Ordner, so reicht der Datei-Name)
    ...
    /**
     * Konstruktor der Klasse FIGUR fuer ausgewaehlte Bilder eines Ordners (im BlueJ-Projekt). Laedt
     * dazu alle Bilder in einem Verzeichnis ein, die zu einem bestimmten Praefix passen. Es wird
     * ein erster Zustand angelegt mit der Animation der Bilder-Folge.
     *
     ...
     * @param praefix Das Praefix, das alle einzuladenden Bilder haben muessen: z.B. "affe" fuer
     *        affe1.jpg, affe2.jpg, ...
     */
     ...
     *
     * @param deltaX Der X Anteil dieser Verschiebung. Positive Werte verschieben nach rechts,
     *        negative nach links.
     *
     * @param deltaY Der Y Anteil dieser Verschiebung. Positive Werte verschieben nach unten,
     *        negative nach oben.
     */
    ...
    /**
     * Diese Methode prueft, wie weit der Mittelpunkt dieses Rechtecks vom Mittelpunkt eines anderen
     * Grfik-Objekts in x-Richtung entfernt ist.
     *
    ...
    /**
     * Diese Methode prueft, wie weit der Mittelpunkt dieses Rechtecks vom Mittelpunkt eines anderen
     * Grfik-Objekts in y-Richtung entfernt ist.
     *
     * @param grafikObjekt Das andere Grafik-Objekt
     *
     * @return Abstand (in Pixeln) dieses Rechtecks vom anderen Grafik-Objekt in y-Richtung (>0,
     *         wenn dieses Rechteck unterhalb des anderen Grafik-Objekts liegt)
     */
    ...
    /**
     * Fuegt einen neuen Zustand <b>mit Spritesheet-Animation</b> ein. Das Spritesheet muss <b>aus
     * Kacheln gleicher Größe</b> bestehen. "leere" Kacheln werden als leere Animationsframes mit
     * einbezogen.
     *
     * @param zustandsName Frei waehlbarer Name des Zustands. (Wird zum Umschalten verwendet)
     * @param bildpfad Pfad zum Spritesheet. (Liegt diese im Projekt-Ordner, so reicht der
     *        Datei-Name)
     ...
     */
    public void fuegeZustandVonSpritesheetHinzu(String zustandsName, String bildpfad, int anzahlX,
                    int anzahlY)
    {
    ...
    /**
     * Fuegt einen neuen Zustand <b>durch einzelne Bilder</b> ein. Die Bilder werden automatisch der
     * Reihe nach durchgewechselt.
     *
     * @param zustandsName Frei waehlbarer Name des Zustands. (Wird zum Umschalten verwendet)
    ...
    /**
     * Fuegt einen neuen Zustand <b>durch ausgewaehlte Bilder eines Ordners</b> ein. Die Bilder
     * werden automatisch der Reihe nach durchgewechselt.
     *
     * @param zustandsName Frei waehlbarer Name des Zustands. (Wird zum Umschalten verwendet)
     * @param verzeichnis Der Name des Ordners (innerhalb des BlueJ-Projekts) in dem die Bilder
     *        liegen
    ...
    /**
     * Versetzt die Figur in einen anderen (bereits existierenden) Zustand. In jedem Fall wird dabei
     * der Animationsloop zurueckgesetzt.
     *
     * @param zustandsName Der Name des zu setzenden Zustands. Unter diesem Namen muss ein Zustand
     *        in dieser Figur existieren.
     */
    ...
    /**
     * Setzt einen automatischen Uebergang von einem Zustand zu einem anderen. Wird der erste
     * Zustand aufgerufen, so wird erst <b>EIN MAL</b> die Animation dieses ersten Zustands
     * aufgerufen und automatisch <b>anschliessend wiederholt</b> die Animation des zweiten
     * Zustands.
     *
     * @param zustandVon Der erste Zustand.
     * @param zustandNach Der zweite Zustand, zu dem die Figur automatisch uebergehen soll, nachdem
     *        die animation des ersten Zustands einmal bis zum Ende durchgelaufen ist.
     */
