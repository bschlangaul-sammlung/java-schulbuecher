 ...
 * Dieser muss sich im Verzeichnis befinden, indem auch die Datei Bild.java (genauer: die Datei
 * Bild.class) liegt. Es muss dort mindestens das Standardbild Tulpe.png vorhanden sein. Versucht
 * der Nutzer eines erfolgreich erzeugten Bildobjekts, einen dateinamen zu setzen, der im
 * Unterordner nicht vorhanden ist, dann gibt es eine kurze freundliche Fehlermeldung auf der
...
public class Bild extends Grafik
{
    ...
    /**
     * Es wird ein neues Bildobjekt mit den Standardeinstellungen erzeugt. Aufpunkt: (0, 0) - linke
     * obere Ecke des ueberdeckenden Rechtecks. Das Standardbild ist eine Tulpe, sie wird angezeigt.
     * Das Bild wird an einem Zeichenfenster angemeldet. Wird null eingegeben, wird das
    ...
    /**
     * Es wird ein neues Bild mit den Standardeinstellungen erzeugt. Dieses kann mit einem
     * Zeichenfensterobjekt verbunden werden und dort sogleich angemeldet werden (-> in den
     * Eingangsparameter anmelden true hineinfuellen) oder nicht (-> in den Eingangsparameter
     * anmelden false hineinfuellen). Wird null in den Eingangsparameter fenster eingefuellt, so
     * wird das Standardfenster verwendet.
     */
    public Bild(boolean anmelden, Zeichenfenster fenster)
    {
        super(anmelden, fenster);
        dateiname = "Tulpe.png";
        internesBild = org.bschlangaul.schulbuecher.paetec.informatik_10_2008.klassen.eps.fortgeschrittene_grafik.interna.BildSpeicher
    ...
    /**
     * Eine neue Bilddatei kann gewaehlt werden. Die Werte der Attribute breite und hoehe werden
     * entsprechend der neuen Bilddatei angepasst.
     *
     * Die Bilddatei muss im Unterordner Bilder (relativ zum Ort der Datei "Bild.class" dieses
     * Projekts) liegen. Es koennen die Formate png und jpg dargestellt werden. Der Unterordner
     * "Bilder/" wird nach der Eingabe automatisch ergaenzt, er darf nicht mit angegeben werden. Der
     * Dienst gibt ggf. Fehlermeldungen auf der Standardkonsole an den Benutzer aus.
     *
     * neuerDateiname - moegliche (Standard)Eingangswerte sind "Kirschen.png" oder "Tulpe.png".
     *
     * Achtung: Die neue Bilddatei wird in Orginalgroesse dargestellt! Die Werte der Attribute
     * breite und hoehe werden entsprechend aktualisiert. Nur die Position (linke obere Ecke des
     * achsenparallelen ueberdeckenden Rechtecks) bleibt erhalten. Alle bisherigen Drehungen,
     * Streckungen (und Veraenderungen der Groesse via groesseSetzen) werden durch das Setzen eines
     * neuen Dateinamens geloescht.
     */
    public void dateinameSetzen(String neuerDateiname)
    {
        Image bild = org.bschlangaul.schulbuecher.paetec.informatik_10_2008.klassen.eps.fortgeschrittene_grafik.interna.BildSpeicher
    ...
    public static String[] bilderGeben()
    {
        URL u = Bild.class.getResource(
                        org.bschlangaul.schulbuecher.paetec.informatik_10_2008.klassen.eps.fortgeschrittene_grafik.interna.BildSpeicher.ordnerpfad);
        ...
        while (it.hasNext())
        {
            if (!org.bschlangaul.schulbuecher.paetec.informatik_10_2008.klassen.eps.fortgeschrittene_grafik.interna.BildSpeicher
     ...
     * Die Grafik ist dann entsprechend auf dem Zeichenfenster zu sehen. Bei einem vorher evt.
     * aufgetretenen Bildfehler wird der Aufruf von groesseSetzen ignoriert (ein nicht existierende
     * Bild kann auch nicht in der Groesse veraendert werden).
     */
    ...
    {
        if (internesBild == null)
            return super.toString() + " - Dateiname: " + dateiname + " - nicht darstellbar";
