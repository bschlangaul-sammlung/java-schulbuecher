...
public class LABOR extends PApplet
{
    ...
    public void setup(){
        ...
        //String dateiname = JOptionPane.showInputDialog(null, "Bitte Dateinamen eingeben:");
        ...
        for(PUNKT p : kmeans.gibDaten()){
            punkte.add(new GPoint((float)p.gibX(), (float)p.gibY(),p.gibBeschriftung()));
        }
        ...
        for(int i = 0; i < kmeans.gibK(); i++){
            ...
            plot.getLayer("zentrum" + (i+1)).setPointColor(color(farbe[0], farbe[1], farbe[2], 200));
            ...
            plot.getLayer("zentrum" + (i+1)).addPoint(new GPoint((float)kmeans.gibZentren()[i].gibX(), (float)kmeans.gibZentren()[i].gibY(),kmeans.gibZentren()[i].gibBeschriftung()));
        }
    ...
    private void iteration(){
        ...
        for(int i = 0 ; i< kmeans.gibAnzahl(); i++){
            cluster[kmeans.gibCluster()[i]-1].add(new GPoint((float)kmeans.gibDaten()[i].gibX(), (float)kmeans.gibDaten()[i].gibY(),kmeans.gibDaten()[i].gibBeschriftung()));
        }
        ...
        for(int i = 0; i < kmeans.gibK(); i++){
            ...
            plot.getLayer("cluster" + (i+1)).setPointColor(color(farbe[0], farbe[1], farbe[2], 150));
        }
        ...
        for(int i = 0; i < kmeans.gibK(); i++){
            int[] farbe = farbWahl(i);
            plot.addLayer("zentrum" + (i+1), zentren[i]);
            plot.getLayer("zentrum" + (i+1)).setPointColor(color(farbe[0], farbe[1], farbe[2]));
            plot.getLayer("zentrum" + (i+1)).setLineColor(100);
    ...
    private int[] farbWahl(int i){
        switch(i){
            case 0:  return new int[]{200,60,0};
            case 1:  return new int[]{60,180,0};
            case 2:  return new int[]{0,120,200};
            case 3:  return new int[]{150,0,200};
            case 4:  return new int[]{200,150,0};
            case 5:  return new int[]{0,140,140};
            case 6:  return new int[]{240,120,180};
            case 7:  return new int[]{240,200,0};
            case 8:  return new int[]{140,180,250};
            case 9:  return new int[]{130,130,130};
