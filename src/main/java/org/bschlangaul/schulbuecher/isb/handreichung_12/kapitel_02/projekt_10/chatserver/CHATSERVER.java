...
/**
 * Implementierung eines Chatservers, Auslagerung der Clientprozesse in einen Thread<br/>
...
public class CHATSERVER
{
    ...
    /**
     * baut eine Verbindung zum Client auf, um dessen Namen zu erfragen. falls die maximale Anzahl
     * der zulässigen Verbindungen erreicht ist, wird die Verbindung nur kurzzeitig aufgebaut, um
     * das Stoppsignal für den Client zu senden
     *
     * @param clientSocket der Socket des Clients
     */
    private synchronized void ClientVerbindungAufbauen(Socket clientSocket) throws IOException
    {
        ...
        if (clientprozesse.size() < maximaleclientanzahl)
        {
            ...
            clientprozess.start();
            System.out.println("neuer Client will sich anmelden / " + clientprozesse.size()
                            + " User angemeldet");
        ...
        else
        {
            ...
            System.out.println("zu viele Verbindungen, Clientverbindung wird zurückgesetzt");
        }
    }

    /**
     * fügt den clientprozess zur Liste der Clientprozesse hinzu und benachrichtigt alle
     * Chatteilnehmer falls die maximale Anzahl der zulässigen Verbindungen erreicht ist, gibt ist
     * der Rückgabewert false und der clientprozess wird beendet
     *
     * @param clientprozess der Clientprozess, der hinzugefügt werden soll
     * @return true bei Erfolg, wird false zurückgegeben, so wird der Clientprozess beendet.
     */
    public synchronized boolean ClientProzessHinzufuegen(CLIENTPROZESS clientprozess)
    {
        ...
        if (clientprozesse.size() < maximaleclientanzahl)
        {
            ...
            AnAlleSenden(clientprozess.SpitznameHolen() + " hat den Raum betreten.");
            System.out.println(clientprozess.SpitznameHolen() + " hat den Raum betreten.");
            System.out.println(clientprozesse.size() + " Verbindungen / " + clientprozesse.size()
                            + " User angemeldet");
            return true;
    ...
    /**
     * entfernt den clientprozess von der Liste der Clientprozesse und benachrichtigt alle
     * Chatteilnehmer
     *
    ...
    public synchronized void ClientProzessEntfernen(CLIENTPROZESS clientprozess)
    {
        ...
        {
            AnAlleSenden(clientprozess.SpitznameHolen() + " hat den Raum verlassen.");
            System.out.println(clientprozess.SpitznameHolen() + " hat den Raum verlassen.");
        }
        System.out.println(clientprozesse.size() + " Verbindungen / " + clientprozesse.size()
                        + " User angemeldet");
    }
     ...
     *
     * @return CLIENTPROZESS clientprozess oder null, falls kein Clientprozess mit einem solchen
     *         Spitznamen gefunden wurde.
     */
     ...
     */
    public void PrivateNachrichtSenden(CLIENTPROZESS clientprozess, String nachricht)
    {
        clientprozess.Senden(nachricht);
    }

    /**
     * versucht den String nachricht an den clientprozess mit dem angegeben Spitznamen zu senden,
     * falls kein Clientprozess mit einem solchen Namen vorhanden ist gibt die funktion false zurück
     *
     ...
     *
     * @param nachricht die Nachricht, die an alle Chatteilnehmer geschickt wird.
     */
