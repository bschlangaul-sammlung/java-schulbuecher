...
class OBERFLAECHE implements OBERFLAECHENINTERFACE
{
    ...
    private void FensterAufbauen()
    {
        ...
        l = new JLabel(" ");
        p.add(l);
        l = new JLabel("Zeit f\u00FCr Sortieren durch Ausw\u00E4hlen [Nanosekunden]");
        p.add(l);
        ...
        auswahlSortieren = new JCheckBox("Sortieren durch Ausw\u00E4hlen testen", true);
        ...
        testen.addActionListener(new ActionListener() {
            ...
            {
                if (auswahlSortieren.isSelected() || mischSortieren.isSelected())
                {
                    try
                    {
                        kontrolleur.SortierenAusfuehren(Integer.parseInt(datenAnzahl.getText()),
                                        auswahlSortieren.isSelected(), mischSortieren.isSelected());
                    } catch (Exception ex)
                    {
                        JOptionPane.showMessageDialog(fenster,
                                        "Bitte korrekte Zahlenwerte angeben.", "Fehlermeldung",
