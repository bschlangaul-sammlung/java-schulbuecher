...
class GraphMatrix
{
    ...
    /**
     * Gibt den Knoten mit den angegebenen Koordinaten zurück. Wenn ein Knoten
     * mit diesen Koordinaten nicht bekannt ist, wird null zurückgegeben
     *
     * @param x x-Koordinate des Knotens
     * @param y y-Koordinate des Knotens
     *
    ...
    /**
     * Gibt die interne Nummer des Knotens Wenn ein Knoten mit diesen
     * Koordinaten nicht bekannt ist, wird -1 zurückgegeben
     *
     * @param x x-Koordinate des Knotens
     * @param y y-Koordinate des Knotens
     *
    ...
    void KanteEinfügen(int x1, int y1, int x2, int y2)
    {
                ...
                && (vonNummer != nachNummer))
        {
            ...
            kanten.add(new KantenSymbol(knoten.get(vonNummer).SymbolGeben(),
                    knoten.get(nachNummer).SymbolGeben(), false, "", 3,
                    "blau"));
        }
    }
    ...
    void KnotenBesuchen(int aktuell, int ziel)
    {
        ...
        if (aktuell == ziel)
        {
            if ((optimalerWeg.size() == 0)
                    || (aktuellerWeg.size() < optimalerWeg.size()))
            {
                optimalerWeg.clear();
