...
class Labyrinth
{
    ...
    void Generieren()
    {
        ...
        while (true)
        {
            ...
            {
                if ((schritte > minSchritte)
                        && (elemente.get(x).get(y) == null))
                {
            ...
            {
                if ((elemente.get(x - 1).get(y - 1) != null)
                        && (elemente.get(x - 1).get(y) != null)
                        && (elemente.get(x).get(y - 1) != null)
                        || (elemente.get(x + 1).get(y - 1) != null)
                                && (elemente.get(x + 1).get(y) != null)
                                && (elemente.get(x).get(y - 1) != null)
                        || (elemente.get(x + 1).get(y + 1) != null)
                                && (elemente.get(x + 1).get(y) != null)
        ...
        for (int x = 0; x < elemente.size(); x += 1)
        {
            for (int y = 0; y < elemente.get(x).size(); y += 1)
            {
                if (!(elemente.get(x).get(y) instanceof Mauer))
                {
                    ...
                    if ((x > 0)
                            && !(elemente.get(x - 1).get(y) instanceof Mauer))
                    {
                        graph.KanteEinfügen(x, y, x - 1, y);
                    }
                    if ((y > 0)
                            && !(elemente.get(x).get(y - 1) instanceof Mauer))
                    {
    ...
    void StartpunktGenerieren()
    {
        ...
        if (Math.abs(x1 - xAusgang) + Math.abs(y1 - yAusgang) > Math
            .abs(x2 - xAusgang) + Math.abs(y2 - yAusgang))
        {
            if (Math.abs(x1 - xAusgang) + Math.abs(y1 - yAusgang) > Math
                .abs(x3 - xAusgang) + Math.abs(y3 - yAusgang))
            {
                xStart = x1;
                yStart = y1;
            }
            else if (Math.abs(x2 - xAusgang) + Math.abs(y2 - yAusgang) > Math
                .abs(x3 - xAusgang) + Math.abs(y3 - yAusgang))
            {
                xStart = x2;
                yStart = y2;
