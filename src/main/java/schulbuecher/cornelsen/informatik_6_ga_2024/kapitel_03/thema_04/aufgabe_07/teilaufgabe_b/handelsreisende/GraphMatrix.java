...
class GraphMatrix
{
                ...
                for (int j = 0; j < matrix.size(); j += 1)
                {
                    if (i != j)
                    {
                        if ((matrix.get(i).get(k) != -1)
                                && (matrix.get(k).get(j) != -1))
                        {
                            if ((matrix.get(i).get(j) == -1)
                                    || (matrix.get(i).get(j) > matrix.get(i)
                                        .get(k) + matrix.get(k).get(j)))
                            {
                                matrix.get(i)
                                    .set(j,
                                        matrix.get(i).get(k)
    ...
    void KnotenBesuchen(int aktuell, int zielKnoten, int länge)
    {
        ...
        for (int nummer = 0; nummer < knoten.size(); nummer += 1)
        {
            if (nummer != aktuell)
            {
                if ((nummer == zielKnoten)
                        && (besuchteKnoten.size() == knoten.size()))
                {
                ...
                else if (!besuchteKnoten.contains(nummer))
                {
                    KnotenBesuchen(nummer,
                        zielKnoten,
                        matrix.get(aktuell).get(nummer) + länge);
                }
            }
        }
    ...
    {
        int nummerBesterStart = -1;
        int längeBesterStart = Integer.MAX_VALUE;
        for (int nummer = 0; nummer < knoten.size(); nummer += 1)
        {
            knoten.get(nummer).FarbeSetzen("gelb");
            besuchteKnoten.clear();
            aktuellerWeg.clear();
            optimalerWeg.clear();
            optimaleLänge = Integer.MAX_VALUE;
            KnotenBesuchen(nummer, nummer, 0);
            if (optimaleLänge < längeBesterStart)
            {
                längeBesterStart = optimaleLänge;
                nummerBesterStart = nummer;
            }
        }
        System.out.println("Der beste Knoten für das Büro ist "
                + knoten.get(nummerBesterStart).BezeichnerGeben());
        System.out.println("Der Weg hat die Länge: " + längeBesterStart);
        knoten.get(nummerBesterStart).FarbeSetzen("grün");
    }
}
