...
class GraphMatrix extends Ereignisbehandlung
{
    ...
    void KantenfarbeSetzen(Knoten k1, Knoten k2, String farbe)
    {
        ...
        {
            if ((k1.SymbolGeben() == k.StartsymbolGeben())
                    && (k2.SymbolGeben() == k.ZielsymbolGeben())
                    || (k2.SymbolGeben() == k.StartsymbolGeben())
                            && (k1.SymbolGeben() == k.ZielsymbolGeben()))
            {
                k.FarbeSetzen(farbe);
            }
        }
    ...
    /**
     * Setzt den Rekursionsschritt bei einem Knoten um. Alle Wege werden
     * gesucht.
     *
    ...
    void KnotenBesuchen1(int aktuell, int ziel)
    {
        ...
        else
        {
            for (int nummer = knoten.size() - 1; nummer >= 0; nummer -= 1)
            {
                ...
                {
                    KantenfarbeSetzen(knoten.get(aktuell),
                        knoten.get(nummer),
                        "grün");
    ...
    /**
     * Setzt den Rekursionsschritt bei einem Knoten um. Der optimale Weg wird
     * gespeichert.
     *
     * @param aktuell die Nummer des aktuell zu besuchenden Knotens
     * @param ziel die Nummer des zu suchenden Knotens
            ...
            for (int nummer = 0; nummer < knoten.size(); nummer += 1)
            {
                        ...
                        && (!besuchteKnoten.contains(knoten.get(nummer))))
                {
                    KantenfarbeSetzen(knoten.get(aktuell),
                        knoten.get(nummer),
                        "grün");
                    KnotenBesuchen2(nummer,
                        ziel,
                        länge + matrix.get(aktuell).get(nummer));
                    KantenfarbeSetzen(knoten.get(aktuell),
                        knoten.get(nummer),
                        "blau");
                }
