package schulbuecher.cornelsen.informatik_6_ga_2024.kapitel_03.thema_04.aufgabe_02.teilaufgabe_c.vertreter;
...
class GraphMatrix
{
     ...
     */
    void KnotenEinfügen(String bezeichner, int x, int y, boolean kunde,
            boolean laden)
    {
     ...
     *
     * @return wahr, wenn ein geeigneter Knoten gefunden wurde
     */
    boolean KnotenBesuchen(int aktuell, String kriterium)
    {
        besuchteKnoten.add(aktuell);
        knoten.get(aktuell).FarbeSetzen("grün");
        if (knoten.get(aktuell).IstLadenVorhanden() && kriterium.equals("laden")
                || knoten.get(aktuell).IstKundeVorhanden()
                        && kriterium.equals("kunde"))
        {
            return true;
        }
        else
        {
            for (int nummer = 0; nummer < knoten.size(); nummer += 1)
            {
                ...
                {
                    if (KnotenBesuchen(nummer, kriterium))
                    {
                        return true;
                    }
                }
     ...
     * @param kriterium das Suchkriterium
     */
    void TiefensucheAusführen(int startKnoten, String kriterium)
    {
        besuchteKnoten.clear();
        KnotenBesuchen(startKnoten, kriterium);
    }
}
