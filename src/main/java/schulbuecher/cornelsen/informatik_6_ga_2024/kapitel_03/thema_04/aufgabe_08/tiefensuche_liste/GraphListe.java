...
class GraphListe
{
    ...
    void KanteEinfügen(String von, String nach, int gewichtung)
    {
        ...
        {
            knoten.get(vonNummer)
                .AdjazenzlisteGeben()
                .add(new Kante(gewichtung, knoten.get(nachNummer)));
            knoten.get(nachNummer)
                .AdjazenzlisteGeben()
                .add(new Kante(gewichtung, knoten.get(vonNummer)));
    ...
    void Ausgeben()
    {
        for (Knoten k : knoten)
        {
            ...
            {
                System.out.print("(" + ka.GewichtGeben() + "-> "
                        + ka.ZielGeben().BezeichnerGeben() + ") ");
            }
            System.out.println();
        }
    }
    ...
    void Umwandeln(ArrayList<ArrayList<Integer>> matrix)
    {
        ...
        {
            for (int spalte = zeile + 1; spalte <= matrix.get(zeile)
                .size(); spalte += 1)
            {
                int gewichtung = matrix.get(zeile).get(spalte);
                knoten.get(zeile)
                    .AdjazenzlisteGeben()
                    .add(new Kante(gewichtung, knoten.get(spalte)));
                knoten.get(spalte)
                    .AdjazenzlisteGeben()
                    .add(new Kante(gewichtung, knoten.get(zeile)));
                kanten.add(new KantenSymbol(knoten.get(zeile).SymbolGeben(),
                        knoten.get(spalte).SymbolGeben(), false,
                        "" + gewichtung, 3, "blau"));
            }
        }
    }
    ...
    void KnotenBesuchen(Knoten aktuell, Knoten zielKnoten, int länge)
    {
        ...
        else
        {
            for (Kante ka : aktuell.AdjazenzlisteGeben())
            {
                ...
                {
                    KnotenBesuchen(ka.ZielGeben(),
                        zielKnoten,
                        ka.GewichtGeben() + länge);
                }
            }
        }
