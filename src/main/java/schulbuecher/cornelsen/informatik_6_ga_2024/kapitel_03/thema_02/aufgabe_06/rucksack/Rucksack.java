package schulbuecher.cornelsen.informatik_6_ga_2024.kapitel_03.thema_02.aufgabe_06.rucksack;
...
class Rucksack
{
    ...
    {
        System.out.println("Der optimale Vorrat hat den Gesamtnährwert "
                + optimalerNährwert + "MJ");
        System.out.println("Einzelvorräte");
        for (Integer i : optimaleVorräte)
        {
            System.out.println("Nummer " + i + ": Gewicht " + gewichte.get(i)
                    + "kg, Nährwert: " + nährwerte.get(i) + "MJ");
        }
    }
     ...
     */
    private void SchrittAusführen(int startIndex, int aktuellesGewicht,
            int aktuellerNährwert)
    {
        for (int index = startIndex; index < gewichte.size(); index += 1)
        {
            if (gewichte.get(index) + aktuellesGewicht <= maximalGewicht)
            {
                aktuelleVorräte.add(index);
                if (nährwerte.get(index)
                        + aktuellerNährwert > optimalerNährwert)
                {
