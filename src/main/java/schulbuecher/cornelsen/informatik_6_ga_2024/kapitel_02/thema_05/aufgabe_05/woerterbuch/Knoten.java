package schulbuecher.cornelsen.informatik_6_ga_2024.kapitel_02.thema_05.aufgabe_05.woerterbuch;

/**
 * Klasse Knoten als Bestandteil eines geordneten Binärbaums. (Implementierung
 * mit Entwurfsmuster Kompositum)
 *
...
class Knoten extends Baumelement
{
     ...
     *
     * @param datenNeu Referenz auf ein Objekt der KLasse Woerterbucheintrag,
     *     das vom Knoten verwaltet wird.
     */
     ...
     */
    Knoten(Woerterbucheintrag datenNeu, Baumelement linkerNachfolgerNeu,
    ...
    /**
     * Fügt ein Datenelement (Woerterbucheintrag), falls sein Schlüssel noch
     * nicht vorhanden ist, in die bestehende Datenstruktur ein. D.h. die
     * Eigenschaft geordneter Binärbaum bleibt erhalten.
     *
    ...
    /**
     * Sucht ein Datenelement (Woerterbucheintrag), das über seinen Schlüssel
     * identifiziert wird und gibt dieses aus.
     *
     * @param suchSchlüssel Schlüssel nach dem gesucht wird als Zeichenkette
     *
     * @return gesuchtes Datenelement (Woerterbucheintrag) bzw. null, falls die
     *     Suche erfolglos ist
     */
    @Override
    ...
    /**
     * Überprüft, ob ein Datenelement (Woerterbucheintrag) mit dem eingegebenen
     * Schlüssel (in Form einer Zeichenkette) vorhanden ist.
     *
     * @param suchSchlüssel Schlüssel, nach dem gesucht wird als Zeichenkette
     *
    ...
    boolean IstVorhanden(String suchSchlüssel)
    {
        ...
        else
        {
            if (daten.SchlüsselIstGrößerAls(suchSchlüssel))
            {
                return linkerNachfolger.IstVorhanden(suchSchlüssel); // Rückgabewert
                                                                     // wird
                                                                     // nach
                                                                     // unten
                                                                     // abgefragt
            ...
            {
                return rechterNachfolger.IstVorhanden(suchSchlüssel); // Rückgabewert
                                                                      // wird
                                                                      // nach
                                                                      // unten
                                                                      // abgefragt
     ...
     */
    @Override
    int TiefeGeben(String suchSchlüssel, int tiefe)
    {
        if (daten.SchlüsselIstGleich(suchSchlüssel))
        {
            return tiefe;
     ...
     */
    @Override
    void StrukturAusgeben(int tiefe)
    {
        // lokales Attribut s: Zeichenkette, mit 4*Tiefe des Knotens
        // Leerzeichen,
     ...
     */
    @Override
    void StrukturAusgeben2(int tiefe)
    {
     ...
     */
    @Override
    Baumelement RechtsEinfügen(Baumelement element)
    {
     ...
     * hat.
     *
     * @param suchSchlüssel Schlüssel, identifiziert das zu entfernende
     *     Datenelement und damit auch den zu entfernenden Knoten
     *
     * @return (neues) Nachfolgeelement des Aufrufers
     */
    @Override
    Baumelement Entfernen(String suchSchlüssel)
    {
        // Datenelement mit suchSchlüssel ist vorhanden -
        // Rekursionsende:zugehöriger Knoten muss entfernt werden
        if (daten.SchlüsselIstGleich(suchSchlüssel))
        {
            // Fall 1: Der zu entfernende Knoten ist ein Blatt
            if (linkerNachfolger instanceof Abschluss
                    && rechterNachfolger instanceof Abschluss)
            {
                return linkerNachfolger; // Knoten wird einfach entfernt->
                                         // Rückgabe eines Abschlusses
            }
            else // Fall 2: Der zu entfernende Knoten hat genau einen Nachfolger
            if (linkerNachfolger instanceof Abschluss)
            {
                return rechterNachfolger; // Knoten wird entfernt und durch
                                          // seinen
                // rechten Nachfolger ersetzt
            }
            else if (rechterNachfolger instanceof Abschluss)
            {
                return linkerNachfolger; // Knoten wird entfernt und durch
                                         // seinen
            ...
            else // Fall 3: Der zu entfernende Knoten hat genau zwei Nachfolger
            {
                ...
                // Entferne das Minimum im rechten Teilbaum und hänge den
                // veränderten
        ...
        // Eigener Schlüssel ist kleiner -> rekursiver Aufruf im rechten
        // Teilbaum
     ...
     */
    @Override
    Baumelement Entfernen2(String suchSchlüssel)
    {
        if (daten.SchlüsselIstGleich(suchSchlüssel))
        {
        ...
        // letzer Fall: Eigener Schlüssel ist größer -> rekursiver Aufruf im
        // linken Teilbaum
        else
        {
            linkerNachfolger = linkerNachfolger.Entfernen2(suchSchlüssel);
     ...
     *
     * @param aktuellesMinimum - die eigenen Daten (dient zur Übergabe an das
     *     Abschluss-Objekt)
     *
     * @return Datenelement mit minimalem Schlüsselwert
     */
    @Override
    Woerterbucheintrag MinimumGeben(Woerterbucheintrag aktuellesMinimum)
    {
